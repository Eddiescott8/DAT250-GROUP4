...
public class IoTDeviceDao {
	...
	public List<IoTDevice> getPopularDevices() {
        Query query = em.createQuery("SELECT d FROM IoTDevice d ORDER BY d.numFollowers LIMIT 10");
        List<IoTDevice> devices = new ArrayList<IoTDevice>();
        devices = query.getResultList();
        return devices;
    }
	...
	public List<IoTDevice> getTopRatedDevices() {
        Query query = em.createQuery("SELECT d FROM IoTDevice d ORDER BY d.averageRating LIMIT 10");
        List<IoTDevice> devices = new ArrayList<IoTDevice>();
        devices = query.getResultList();
        return devices;
    }
	...
	@SuppressWarnings("unchecked")
	public List<IoTDevice> getDeviceByCategory(String category) {
        Query query = em.createQuery("SELECT d FROM IoTDevice d");
        List<IoTDevice> devices = new ArrayList<IoTDevice>();
        devices = query.getResultList();
        return devices;
